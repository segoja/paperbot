...
export default class PouchAdapter extends RESTAdapter {
  ...
  _init(store, type, indexPromises) {
    // // console.debug('_init: ', type);
    var self = this,
  ...
  _recordToData(store, type, record) {
    var data = {};
  ...
  getRecordTypeName(type) {
    // // console.debug("getRecordTypeName: ",type.modelName);
    return decamelize(type.modelName);
  }

  async findAll(store, type /*, sinceToken */) {
    // TODO: use sinceToken
    // console.debug('findAll type: ', type);
   ...
   *  so instead of record.type we have to use store.modelFor
   */

  async findHasMany(store, record, link, rel) {
    // console.debug('findHasMany record: ', record);
    // console.debug('rel.type: ', rel);
    let recordModelClass =
      typeof record.type === 'string'
        ? record.type
        : store.modelFor(record.modelName);
    // // console.debug('recordModelClass: ', recordModelClass);
    await this._init(store, recordModelClass);
    let inverse = recordModelClass.inverseFor(rel.key, store);
    // console.debug('inverse: ',inverse);
    if (inverse && inverse.kind === 'belongsTo') {
      return this.db.rel.findHasMany(rel.type, inverse.name, record.id);
    } else {
      let result = {};
