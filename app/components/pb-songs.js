...
export default class PbSongsComponent extends Component {
  ...
  @action songImport(file){
    ...
    reader.readAsText(file.blob).then((text) => {    
      let reference = ["title","artist","type","active","admin","mod","vip","sub","date_added","last_played","times_requested","times_played"];
  ...
  @action songExportFiltered() {
    ...
    if (this.filteredContent !== 0){
      let header = ["title","artist","type","active","admin","mod","vip","sub","date_added","last_played","times_requested","times_played"];
      csvdata.push(header);
      this.filteredContent.forEach((song) => {
        let csvrow = [song.title,song.artist,song.type,song.active,song.admin,song.mod,song.vip,song.sub,song.date_added,song.last_played,song.times_requested,song.times_played];
        csvdata.push(csvrow);
      });
    }
