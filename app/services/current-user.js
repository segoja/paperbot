...
export default class CurrentUserService extends Service {
  ...
  @action importRecords(reference,extension,recordType,response){
    ...
    /*if(this.isTauri){
      ...
      if(response){
        let rows = PapaParse.parse(response,{ delimiter: ',', header: true, quotes: false, quoteChar: '"', skipEmptyLines: true }).data;
  ...
  @action download(data, filename, type) {    
    ...
    if(this.isTauri){
      dialog.save({
        ...
        if(path){
          await invoke('file_writer', { filepath: path, filecontent: data}).then(()=>{
